...
//    gl_FragColor.xyz = col.xyz;
    ...
    gl_FragColor = col*1.0f;
    gl_FragColor = mix(0.5*vec4(pow(length(col.xyz*1.0f-vec3(0.5)),1.0)),vec4(1),whiter)*fader;

    // for shillouette
    //gl_FragColor = vec4(0,0,0,1)*(1.0-3*0.5*vec4(pow(length(col.xyz*1.0f-vec3(0.5)),1.0)));
...
//    gl_FragColor.w = 4.0*length(col.xy-vec2(0.5))*length(col.xy-vec2(0.5))*alpha;
    ...
    gl_FragColor.w = mix(1.5*(length(col.xy))*length(col.xy)*alpha,1,whiter)*fader*modeFader;
