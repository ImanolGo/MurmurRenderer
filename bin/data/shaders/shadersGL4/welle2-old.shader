...
//    float stepnum = 8.0;
    ...
    vec2 randTexCoord = texc*res/randres+rand;
...
//    vdelta=vec2(0.002);
    ...
    vp0 = texture2D( s_tex_0, texc2.xy+vdelta*vec2( 1.0, 0.0) );
    vm0 = texture2D( s_tex_0, texc2.xy+vdelta*vec2(-1.0, 0.0) );
    v0p = texture2D( s_tex_0, texc2.xy+vdelta*vec2( 0.0, 1.0) );
    v0m = texture2D( s_tex_0, texc2.xy+vdelta*vec2( 0.0,-1.0) );
    vpp = texture2D( s_tex_0, texc2.xy+vdelta*vec2( 1.0, 1.0) );
    vmm = texture2D( s_tex_0, texc2.xy+vdelta*vec2(-1.0,-1.0) );
    vmp = texture2D( s_tex_0, texc2.xy+vdelta*vec2(-1.0, 1.0) );
    vpm = texture2D( s_tex_0, texc2.xy+vdelta*vec2( 1.0,-1.0) );
